...
public interface GetAllMemoRepository extends JpaRepository<GetAllMemo, Integer>{
	...
	@Query(value="SELECT m.memo_header_id,m.memo_no,d.driver_license_no, o.owner_name, d.driver_name,m.created_date, m.office_id,m.to_id, m.staff_id, m.veh_id, v.veh_no ,m.driver_id, cl1.office_name AS from_office, cl2.office_name AS to_office, cl1.office_address " + 
			...
			"INNER JOIN m_office cl2 ON cl2.office_id = m.to_id " + 
			"WHERE m.driver_id=d.driver_id AND m.veh_id=v.veh_id AND v.owner_id=o.owner_id " + 
			"AND YEAR(m.created_date)=YEAR(CURRENT_DATE()) AND MONTH(m.created_date)=MONTH(CURRENT_DATE()) AND m.memo_header_id=:memoHeaderId AND m.is_used=true",nativeQuery=true)
	...
	@Query(value="SELECT m.memo_header_id,m.memo_no,d.driver_license_no, o.owner_name, d.driver_name,m.created_date, m.office_id,m.to_id, m.staff_id, m.veh_id, v.veh_no ,m.driver_id, cl1.office_name AS from_office, cl2.office_name AS to_office, cl1.office_address " + 
			...
			"INNER JOIN m_office cl2 ON cl2.office_id = m.to_id " + 
			"WHERE m.driver_id=d.driver_id AND m.veh_id=v.veh_id AND v.owner_id=o.owner_id " + 
			"AND YEAR(m.created_date)=YEAR(CURRENT_DATE()) AND MONTH(m.created_date)=MONTH(CURRENT_DATE()) AND m.memo_no=:memoNo AND m.status=0 AND m.is_used=true",nativeQuery=true)
	GetAllMemo findByMemoNo(@Param("memoNo")int memoNo);
	...
	@Query(value="SELECT m.memo_header_id,m.memo_no,d.driver_license_no, o.owner_name, d.driver_name,m.created_date, m.office_id,m.to_id, m.staff_id, m.veh_id, v.veh_no ,m.driver_id, cl1.office_name AS from_office, cl2.office_name AS to_office, cl1.office_address " + 
			...
			"INNER JOIN m_office cl2 ON cl2.office_id = m.to_id  " + 
			"WHERE m.driver_id=d.driver_id AND m.veh_id=v.veh_id AND v.owner_id=o.owner_id " + 
			"AND YEAR(m.created_date)=YEAR(CURRENT_DATE()) AND MONTH(m.created_date)=MONTH(CURRENT_DATE()) AND m.status=1 AND m.is_used=TRUE AND m.office_id=:officeId",nativeQuery=true)
	List<GetAllMemo> findAllReceivedMemo(@Param("officeId")int officeId);
