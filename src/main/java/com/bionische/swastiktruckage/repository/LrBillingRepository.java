...
public interface LrBillingRepository extends JpaRepository<LrBilling,Integer>{
	...
	@Query(value="SELECT m.client_name AS consignee_name,(SELECT client_name FROM m_clients WHERE client_id=h.consignor) AS consignor_name,(SELECT goods FROM t_lr_containt_details WHERE (lr_header_id,no_of_containts) IN (SELECT lr_header_id,MAX(no_of_containts) FROM t_lr_containt_details WHERE lr_header_id=h.lr_header_id ) ORDER BY goods ASC LIMIT 1)AS goods, h.lr_header_id,h.consignee_id,h.consignor,h.lr_No,h.inv_header_id,h.from_id,h.lr_date,h.truck_no,h.weight,h.freight,h.gst,h.hamali,h.local_tempo,h.total,h.payment_by, \r\n" + 
	...
	@Query(value="SELECT l.lr_header_id,l.lr_No,l.inv_header_id,l.from_id,l.lr_date,l.truck_no,l.weight,l.freight,l.gst,l.hamali,l.local_tempo,l.total,l.payment_by,SUM(c.no_of_containts) AS quantity,l.consignor,l.consignee_id,m.client_name AS consignee_name,(SELECT client_name FROM m_clients WHERE client_id=l.consignor) AS consignor_name,\r\n" + 
			"(SELECT goods FROM t_lr_containt_details WHERE (lr_header_id,no_of_containts) IN (SELECT lr_header_id,MAX(no_of_containts) FROM t_lr_containt_details WHERE lr_header_id=l.lr_header_id ) ORDER BY goods ASC LIMIT 1)AS goods from t_lr_header l,t_bill_header b,t_bill_details d,t_lr_containt_details c,m_clients m WHERE b.bill_status=0 AND b.bill_header_id=d.bill_header_id AND d.lr_header_id=l.lr_header_id \r\n" + 
